...
int main(int argc, char* argv[])
{
  ...
  gripper_pose *= Eigen::AngleAxisd(-M_PI/2, Eigen::Vector3d::UnitZ());
  gripper_pose.translation() = Eigen::Vector3d(grid_size/2, 0.02, 0.156+4*grid_size);
  visual_tools_->publishMesh(gripper_pose, "package://renbo_description/meshes/arm/r_gripper.stl", rviz_visual_tools::WHITE, rviz_visual_tools::XXXXSMALL);
  ...
  cylinder_pose.translation() = Eigen::Vector3d(0.4, 0.0, 0.06);

  geometry_msgs::Pose cylinder_pose_msgs;
  tf::poseEigenToMsg(cylinder_pose, cylinder_pose_msgs);

  visual_tools_->publishCylinder(cylinder_pose_msgs, getColor(176.0, 196.0, 222.0, 1.0), 0.12, 0.05);

  Eigen::Affine3d cuboid_pose;
  cuboid_pose = Eigen::AngleAxisd(0.0, Eigen::Vector3d::UnitX());
  cuboid_pose.translation() = Eigen::Vector3d(object_size(0)/2, object_size(1)/2, object_size(2)/2);
...
//  visual_tools_->trigger();
  ...
  cuboid_plane_pose.translation() = Eigen::Vector3d(tmp(0), tmp(1) - grid_size/2-0.001, tmp(2)+3*grid_size);
  visual_tools_->publishCuboid(cuboid_plane_pose, 0.019, 0.0001, 0.029, getColor(0.0, 191.0, 255.0, 1.0));
  cuboid_plane_pose.translation() = Eigen::Vector3d(tmp(0), tmp(1) + 3*grid_size/2+0.001, tmp(2)+3*grid_size);
  visual_tools_->publishCuboid(cuboid_plane_pose, 0.019, 0.0001, 0.029, getColor(0.0, 191.0, 255.0, 1.0));
